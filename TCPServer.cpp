...
public :
       ...
       {      
              ...
              // .c_str() convert string into character array
              if(startServer() != 0){
       ...
       void startListen(){
              ...
              while(true){
                     ...
                     std::string inputStr(buffer);  // Convert received char[] to string
                     log("Received from client and enter command: " + inputStr);

                     // 🧠 Split first word as command
                     ...
                     if (command == "SORT")
                     {
                            ...
                            mergeSort(nums, 0, nums.size() - 1);  // Sorting algorithm
